  ...
  return (
    <section className={`${styles.flexCenter}  ${styles.paddingY} flex-col`}>
            ...
            '>
                {footerLinks.map((footerLink)=>{
                    return(
                      ...
                      <h4 className='font-poppins font-mediu text-[18px] leading-[27px] text-white'>{footerLink.title}</h4>
        ...
        {socialMedia.map((social, index)=>{
            return(
                <img key={social.id} src={social.icon} alt={social.id}  className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !== socialMedia.length-1 ?
                     'mr-6': 'mr-0'}`}
